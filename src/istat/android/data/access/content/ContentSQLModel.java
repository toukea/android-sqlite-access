...
import istat.android.data.access.content.interfaces.JSONable;
import istat.android.data.access.content.interfaces.QueryAble;
import istat.android.data.access.content.utils.Toolkit;
...
public abstract class ContentSQLModel implements JSONable, QueryAble, Cloneable, Iterable {
    ...
    public static final String TAG_CLASS = ContentSQLModel.class.getCanonicalName() + ".CLASS";
    public static final String TAG_ITEMS = ContentSQLModel.class.getCanonicalName() + ".ITEMS";
    ...
    public final void fillFromPrimaryKey(String primaryKey, ContentResolver db) {
    ...
    public void refresh(ContentResolver db) {
        fillFromPrimaryKey(getPrimaryKeyStringValue(), db);
    }

    public long merge(ContentResolver db) {
        if (exist(db)) {
            long out = update(db);
            refresh(db);
            return out >= 1 ? 0 : -1;
    ...
    public long persist(ContentResolver db) {
        if (exist(db)) {
            long out = update(db);
            return out >= 1 ? 0 : -1;
        } else {
    ...
    protected void onPersistEmbeddedDbEntity(ContentResolver db, Class cLass, ContentSQLModel embeddedModel) throws SQLiteException {
        embeddedModel.persist(db);
    }

    @Override
    public long insert(ContentResolver db) throws SQLiteException {
        return insert(db, false);
    }

    private long insert(ContentResolver db, boolean updateModelPrimaryKey) throws SQLiteException {
        long insertId;
...
//            setPrimaryKeyValue(primaryKeyValue);
    ...
    public int update(ContentResolver db) throws SQLiteException {
        int out;
        try {
    ...
    public void merge(ContentSQLModel entity) {
        merge(entity, true, true);
    }

    public void merge(ContentSQLModel entity, boolean override, boolean mergeEmptyValue) {
    ...
    public boolean exist(ContentResolver db) {
        String primary_key_name = getPrimaryKeyName();
    ...
    public int delete(ContentResolver db) {
        return delete(db, getPrimaryKeyName() + "= ?",
                new String[]{getPrimaryKeyStringValue()});
    }

    public static ContentSQLModel fromJson(final String tableName, final String primaryKeyName, final String[] columns, JSONObject json) {
        ContentSQLModel model = new ContentSQLModel() {
            @Override
    ...
    public static ContentSQLModel fromObject(final Object obj) throws InstantiationException,
            IllegalAccessException {
        return fromObject(obj, ContentSQLModel.DEFAULT_SERIALIZER, DEFAULT_CONTAIN_VALUE_HANDLER);
    }

    @SuppressWarnings("unchecked")
    public static ContentSQLModel fromObject(final Object obj, Serializer serializer, ContentValueHandler contentValueHandler) throws InstantiationException,
            IllegalAccessException {
        try {
            if (obj instanceof ContentSQLModel) {
                ContentSQLModel model = (ContentSQLModel) obj;
                return (ContentSQLModel) model.clone();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//    public final static <T, Y> ContentSQLModel fromMap(String tableName, Map<T, Y> map) {
//
//    }
    ...
    public static ContentSQLModel fromClass(final Class cLass) throws InstantiationException,
            IllegalAccessException {
        return fromClass(cLass, ContentSQLModel.DEFAULT_SERIALIZER, DEFAULT_CONTAIN_VALUE_HANDLER);
    }

    public static ContentSQLModel fromClass(Class cLass, Serializer serializer, ContentValueHandler contentValueHandler) throws InstantiationException,
            IllegalAccessException {
        if (serializer == null) {
    ...
    private void persistNestedEntity(ContentResolver db) {
        try {
    ...
    private void traitNestedTableField(Field field, ContentResolver db) throws IllegalAccessException, InstantiationException {
        Class<?> cLass = Toolkit.getFieldTypeClass(field);
        ContentSQLModel model = ContentSQLModel.fromObject(cLass);
    ...
    protected int update(ContentResolver db, String whereClause,
                         String[] whereArgs) {
        return db.update(getName(), toContentValues(), whereClause,
                whereArgs);
    }

    protected int delete(ContentResolver db, String whereClause,
                         String[] whereArgs) {
    ...
    public final static class Builder {
        Serializer serializer = ContentSQLModel.DEFAULT_SERIALIZER;
...
//        }
        ...
        public ContentSQLModel create() {
            return create(null);
        }

        public ContentSQLModel create(HashMap<String, Object> fieldNameValuePair) {
            ContentSQLModel model = new ContentSQLModel() {
                String[] columns;
    ...
    final static ContentSQLModel createFromManyToMany(Class<?> parentTableClass, Class<?> childTableClass) throws InstantiationException, IllegalAccessException {
        ContentSQLModel parentModel = getSQLiteModel(parentTableClass);
        ContentSQLModel childModel = getSQLiteModel(childTableClass);
    ...
    private static ContentSQLModel getSQLiteModel(Class<?> childTableClass) throws IllegalAccessException, InstantiationException {
        return ContentSQLModel.fromClass(childTableClass);
    }

    @Override
    public boolean equals(Object o) {
        //TODO compare value as ContentSQL data.
        return super.equals(o);
    }

    /**
     * specify a way to read and fill ContentSQLModel from ContentResolver Cursor.
     *
     * @param cursorReader
     */
    ...
    static final CursorReader DEFAULT_CURSOR_READER = new CursorReader() {
        @Override
        public void onReadCursor(ContentSQLModel model, Cursor c) {
...
//                            DEFAULT_SERIALIZER.onDeSerialize(values,model.getField(projection))
    ...
    static ContentValueHandler DEFAULT_CONTAIN_VALUE_HANDLER = new ContentValueHandler() {
        @Override
        public ContentValues toContentValues(ContentSQLModel model) {
                ...
                if (column != null) {
                    ...
                    if (column.equals(model.getPrimaryKeyName())) {
                        ...
                        if ((model.getPrimaryKeyPolicy() == PrimaryKey.POLICY_AUTO_INCREMENT
                            ...
                            Log.d("ContentSQLModel", "toContentValues:" + column + " is primary key should be autoIncremented.");
...
//                        }
    ...
    public interface CursorReader {
        void onReadCursor(ContentSQLModel model, Cursor cursor);
    }

    public interface ContentValueHandler {
        ContentValues toContentValues(ContentSQLModel model);
